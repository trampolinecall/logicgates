...
use crate::simulation::{logic, CircuitKey, GateKey, NodeKey, Simulation, self};
...
pub(crate) fn render(app: &App, draw: &Draw, simulation: &Simulation, main_circuit: CircuitKey) {
    ...
    let gate_inputs = main_circuit.gates.iter().flat_map(|gk| simulation::gate_inputs(&simulation.circuits, &simulation.gates, *gk).iter().enumerate().map(|(i, k)| (NodePos::GI(*gk, i), *k)));
    ...
    for gate_i in &main_circuit.gates {
        ...
        draw.text(simulation.gates[*gate_i].name(&simulation.circuits)).xy(rect.xy()).wh(rect.wh()).center_justify().align_text_middle_y();
    }
    ...
    let circuit_outputs = main_circuit.outputs.iter().enumerate().map(|(i, k)| (NodePos::CO(simulation.main_circuit, i), *k));
    let gate_inputs = main_circuit.gates.iter().flat_map(|gk| simulation::gate_inputs(&simulation.circuits, &simulation.gates, *gk).iter().enumerate().map(|(i, k)| (NodePos::GI(*gk, i), *k)));
    let gate_outputs = main_circuit.gates.iter().flat_map(|gk| simulation::gate_outputs(&simulation.circuits, &simulation.gates, *gk).iter().enumerate().map(|(i, k)| (NodePos::GO(*gk, i), *k)));
...
fn gate_rect(window_rect: Rect, simulation: &Simulation, gate_index: GateKey) -> Rect {
    let location = &simulation.gates[gate_index].location(&simulation.circuits); // TODO: this should eventually be the center
...
pub(crate) fn gate_display_size(simulation: &Simulation, gate: GateKey) -> Vec2 {
    ...
    let gate_height = (std::cmp::max(simulation::gate_num_inputs(&simulation.circuits, &simulation.gates, gate), simulation::gate_num_outputs(&simulation.circuits, &simulation.gates, gate)) - 1) as f32
