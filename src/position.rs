...
pub(crate) fn calculate_locations(circuit: &circuit::Circuit) -> HashMap<circuit::GateIndex, (u32, f64)> {
    ...
    */
    ...
    for (gate_i, gate) in circuit.gates.iter() {
        let input_producer_x = |input: circuit::GateInputNodeIdx| match circuit.get_value_receiving_node(input.into()).producer {
            Some(producer) => match circuit.get_value_producing_node(producer).gate {
    ...
    for x in 1..=*xs.values().max().unwrap_or(&0) {
        let input_producer_y = |input: circuit::GateInputNodeIdx| match circuit.get_value_receiving_node(input.into()).producer {
            Some(producer) => match circuit.get_value_producing_node(producer).gate {
